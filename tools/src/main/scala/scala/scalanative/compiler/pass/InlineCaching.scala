...
class InlineCaching(dispatchInfo: Map[String, Seq[Int]],
  ...
  private def addInlineCaching(block: Block): Seq[Block] = {
    if (block.insts exists isVirtualDispatch) {
      val (init,
           inst @ Let(n, Op.Method(_, obj, MethodRef(cls: Class, meth))),
  ...
  def apply(ctx: Ctx) =
    ctx.options.profileInfo match {
      case Some(info) if info.exists =>
        val maxCandidates = ctx.options.inlineCachingMaxCandidates
        val dispatchInfo =
          tools.DispatchInfoParser(Source.fromFile(info).mkString)
        new InlineCaching(dispatchInfo, maxCandidates)(ctx.fresh, ctx.top)
