...
class MethodLowering(implicit fresh: Fresh, top: Top) extends Pass {
  override def preInst = {
    case Let(n, Op.Method(packed, MethodRef(cls: Class, meth)))
        if meth.isVirtual =>
    ...
    case Let(n, Op.Method(_, MethodRef(_: Class, meth))) if meth.isStatic =>
      Seq(
        Let(n, Op.Copy(Val.Global(meth.name, Type.Ptr)))
      )

    case Let(n, Op.Method(packed, MethodRef(trt: Trait, meth))) =>
      val typeptr    = Val.Local(fresh(), Type.Ptr)
