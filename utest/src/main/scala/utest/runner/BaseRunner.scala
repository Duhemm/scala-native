...
abstract class BaseRunner(val args: Array[String],
  ...
  def runSuite(selector: Seq[String],
    ...
    implicit val ec =
      if (utest.framework.ArgParse
            .find("--parallel", _.toBoolean, false, true)(args)) {
      ...
      (subpath, s) => {
        ...
        s.value match {
          case Failure(e) =>
            ...
            e.setStackTrace(
              e.getStackTrace.takeWhile(
                _.getClassName != "utest.framework.TestThunkTree")
            )
  ...
  def deserializeTask(task: String,
                      deserializer: String => TaskDef): sbt.testing.Task =
    makeTask(deserializer(task))

  def serializeTask(task: sbt.testing.Task,
